...
const { toAudio, toPTT, toVideo } = require('./converter')a
...
`.trim()
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/+6285651307830" },
...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'Hafizh', body: wm, sourceUrl: 'https://instagram.com/hfizh22', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
